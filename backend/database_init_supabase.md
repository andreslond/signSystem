...
This document contains the complete SQL script to initialize the SignSystem database in Supabase.
...
-- Note: This requires actual auth users to exist first
...
-- Note: Profiles must be created by users themselves or via service role
-- Sample documents would be inserted via the application API
*/
...
GRANT SELECT, UPDATE ON ar_signatures.profiles TO authenticated;
GRANT ALL ON ar_signatures.profiles TO service_role;

GRANT SELECT ON ar_signatures.documents TO authenticated;
GRANT ALL ON ar_signatures.documents TO service_role;

GRANT SELECT ON ar_signatures.signatures TO authenticated;
GRANT ALL ON ar_signatures.signatures TO service_role;
...
1. **Copy the SQL code** from the code block above
2. **Open Supabase Dashboard** → Your Project → SQL Editor
3. **Paste and run** the entire script
4. **Verify installation** by running:
   ```sql
   SELECT schemaname, tablename FROM pg_tables WHERE schemaname = 'ar_signatures';
   SELECT * FROM pg_policies WHERE schemaname = 'ar_signatures';
   ```

## Key Features Included

- ✅ **Complete schema** with all tables, constraints, and relationships
- ✅ **Row Level Security** policies for data protection
- ✅ **Optimized indexes** for query performance
- ✅ **Data validation** constraints (date ranges, status checks)
- ✅ **Unique constraints** preventing duplicate active documents
